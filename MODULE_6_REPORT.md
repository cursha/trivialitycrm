...
Deliberately **not** in this migration (would forward-reference tables/features that don't exist yet): anything inbound-sync-related (Phase D2); `EmailMessage.bulkSendBatchId` (a later phase or Module Seven, if bulk send is ever built).

All four migrations applied to dev and test databases; `npx prisma generate` re-run after each. `prisma/seed.ts` re-run against dev after Phase A — **41 permissions seeded (up from 30)**; Phases B, C, and D1 added no new permission keys (D1 reuses `manage_calendar_connections`, already seeded in Phase A for exactly this purpose).
...
Every permission in this matrix was seeded back in Phase A (per the full plan's matrix, added all at once rather than piecemeal per phase). As of Phase D1, every permission gates real UI except `view_team_communications` and `send_bulk_email` (both await Phase D2/E-and-beyond features). `manage_calendar_connections` — despite its name suggesting only "connection management" — is what Phase D1 reuses to gate actually scheduling/updating/cancelling appointments too, since the plan's own matrix describes it as self-scoped ("own") for Manager/Salesperson exactly like the self-service actions it now covers; no new permission key was needed.
...
| Company detail page | "Email" panel (compose/send/schedule + history); Contacts panel shows a consent badge + "Manage consent" link; "Follow-up Sequences" panel (enroll with step preview, pause/resume/cancel); new "Appointments" panel (schedule/reschedule/cancel a demo/trial-review/follow-up) |
| Settings hub (`/settings`) | Four new cards: Email Connections, Email Templates, Communication Compliance, Follow-up Sequences |

No new top-level route for appointments — it's a panel on the existing company detail page, matching how the Email and Follow-up Sequences panels were added rather than a separate calendar page.

## Tests

**517 tests passing (500 from the post-Phase-C-fixes checkpoint, unmodified in substance, + 17 new across 4 test files, from Phase D1)**:
- Every prior test file — unchanged, all still green
- `tests/unit/calendar-time.test.ts` (3 tests, new) — `formatWallClock()` converts a UTC instant to the correct wall-clock string in a target zone, produces a different string for a different zone from the same instant, and correctly crosses a calendar-date boundary near midnight UTC
- `tests/unit/comms-providers.test.ts` (+2 tests) — `MockEmailProvider` creates/updates/cancels a calendar event with no network access, and simulates a calendar provider failure for `SIMULATED_CALENDAR_FAILURE_TITLE` (the only way tests exercise `Appointment`'s `ERROR` path); both real providers' authorization-URL tests now also assert the calendar scope is present
- `tests/integration/appointments.test.ts` (8 tests, new) — `createAppointment()` rejects an end time at or before the start, requires a connected mailbox, creates the row with a real `providerEventId` on success, and keeps an auditable `ERROR` row (never a fake event id) on provider failure; `updateAppointment()` updates fields and sets `UPDATED` on success, fails when the merged times would be invalid; `cancelAppointment()` cancels successfully and is idempotent on an already-cancelled appointment
- `tests/integration/appointment-actions.test.ts` (4 tests, new) — the schedule action enforces `manage_calendar_connections`, rejects an invalid timezone before ever reaching the provider, schedules a real appointment end to end; the cancel action is scoped to companies the caller can actually access

No automated test ever sends real email, creates a real calendar event, or reaches a real OAuth endpoint — `MockEmailProvider` is structurally the only provider active under `NODE_ENV=test`.

**A note on test-suite stability**: across all phases of this module, full-suite runs have intermittently seen one or two *different*, unrelated test files fail with a generic hook/test timeout rather than an assertion failure — this checkpoint it was `tests/integration/rate-limit.test.ts`'s fixed-window boundary test again. Every one has passed cleanly (100%) when re-run in isolation immediately after, including this checkpoint's final run. Consistent with transient Postgres connection contention across Vitest's parallel test-file workers in this sandbox, not a regression — noted here rather than silently ignored.
...
- `npx eslint .` — clean
- `npm test` — 517/517 passing (see the stability note above)
- `npx next build` — succeeds; no new top-level route this phase (appointments is a panel on the existing company detail page), every Phase A/B/C and pre-existing route still registered
- `npm run worker` — **a real bug, found only by actually starting the worker (Phase C)**: `src/lib/comms/connections.ts` still carried Phase A's `import "server-only"` guard, written when only web Server Actions called it. Phase C's `sequence-tick`/`send-scheduled-email` worker handlers now call `sendEmail()` (`send-email.ts`), which calls `getUsableAccessToken()` in `connections.ts` — so the worker crashed at import time the instant it was started for real, exactly the class of bug the Module Five `server-only` lesson describes. Fixed then; re-confirmed clean again this phase — `src/lib/comms/appointments.ts` and `calendar-time.ts` both correctly omit the guard too, and no worker handler imports them yet (Phase D1 has no worker component — appointment scheduling is a direct, synchronous provider call from the web action, not a queued job), so there was nothing new to catch this time, but the check was run anyway rather than assumed.

## Browser walkthrough

**Not performed as an interactive click-through** — no browser automation tool was available this session, same limitation as every prior checkpoint. In its place: `tsc`/lint/the full test suite/a production build all passed, and the worker process was started for real. A manual interactive pass — the appointment scheduling form (including a real timezone edge case), the reschedule inline form, and cancel confirmation — is still recommended before considering Phase D1 fully verified, in addition to the manual passes already recommended for Phases A–C.
...
- **`STOPPED_REPLY` is unreachable.** Reply detection needs inbound email sync, which is Phase D2 — a sequence can be stopped by opt-out or stage-reached today, never by a detected reply.
- **No inbound email sync yet.** Phase D1 only added outbound calendar scheduling; matching inbound replies to contacts, avoiding duplicate imports, and a review UI for unmatched senders are all Phase D2.
...
- **`Notification` rows are surfaced in the dashboard bell (fixed post-Phase-C) but there is no appointment-specific notification yet** — `APPOINTMENT_ACCEPTED`/`APPOINTMENT_DECLINED` exist on `NotificationType` for schema completeness but nothing creates one; detecting an attendee's accept/decline response requires the same inbound-sync/webhook infrastructure as Phase D2/E, not something Phase D1's outbound-only scheduling can produce on its own.
...
The approved plan's remaining phases (D2–E) are the direct next steps for Module Six itself:

1. **Phase D2 — Inbound sync**: webhook subscriptions per provider (Graph's `clientState`+validation-token handshake, Google's signed-JWT Pub/Sub push) plus a subscription-renewal tick (Graph subscriptions expire in ~3 days, Google's `watch()` channels in ~7 — both need periodic renewal, a genuinely new piece of worker infrastructure); contact-matching by normalized email; duplicate-import prevention; `/settings/communications-review` for unmatched senders; reply detection feeding `STOPPED_REPLY` into the sequence engine. Flagged earlier as the single largest remaining phase — the first time this app accepts unsolicited inbound calls from the internet, raising the security bar (idempotency, signature verification) above anything built so far.
2. **Phase E — Delivery status and unified notifications**: provider delivery-status webhooks (idempotent, signature-verified, likely reusing D2's webhook plumbing); generalize the dashboard bell's *storage*, not just its display, so `GeneratedReport` and `Notification` are eventually one system instead of two read from the same bell.
...
4. **Once real OAuth app registrations exist** (Microsoft Entra / Google Cloud Console, not created this session per the plan's "never connect real accounts without approval"): a real manual connect/disconnect cycle, one real send to a controlled test mailbox, one real scheduled send, one real sequence enrollment, one real appointment create/update/cancel (confirming attendees actually receive Graph's/Google's real notifications), and one real unsubscribe click-through, before trusting Phases A–D1 against a live account.
