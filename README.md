...
- **Consent and compliance** (`/settings/communication-compliance`, `manage_communication_compliance`; also reachable from a contact on its company page): `ConsentRecord` (append-only, mirroring Module One's `HistoricalScoreRecord` pattern) tracks each contact's `emailPermitted`/`doNotContact` state, but as of this app's use case (cold outreach to newly discovered leads), that state is **not** enforced as an upfront send gate — only an explicit opt-out is. A linked contact is mandatory for every send (no free-text "To" field, no untracked-address path) — `sendEmail()` checks `Company.doNotContact` *and* `Contact.doNotContact` plus a mandatory, working unsubscribe link, enforced once, inside `prepareSend()`/`sendEmail()` itself (re-run fresh every time a send actually happens, whether immediate, scheduled, or a sequence step), not left to each caller. The public, no-login `/unsubscribe` page records a `WITHDRAWN` consent record and flips `Contact.doNotContact` on click (idempotent). This is a compliance-support system, not legal advice.
...
- **Forgot password** (`/forgot-password`, public): self-service request by email — always shows the same generic message, never reveals whether an account exists. There is no transactional email sender in this app (`src/lib/comms/send-email.ts` is CRM-outreach only), so delivery is admin-mediated: pending requests appear on `/settings/users` (`manage_users`) where an administrator generates a single-use, one-hour reset link (revealed exactly once, only its hash is ever persisted) and relays it to the user out of band, or dismisses the request. Completing a reset at `/reset-password?token=...` sets the user's own new password, clears any lockout, and invalidates every existing session for that account.
