...
## Module Six: Communications and Follow-up Automation (Phases A–D1)

Module Six lets the sales team email leads and schedule appointments from inside the CRM using their own connected mailbox+calendar — see `MODULE_6_REPORT.md` for the full phased plan, provider comparison, permission matrix, and what's still ahead in Phases D2–E. **Phases A, B, C, and D1** (connections, templates, composer, consent/compliance, scheduled sends, follow-up sequences, calendar/appointments) are built — inbound sync and delivery-status webhooks are designed but not yet built.
...
Not yet in Module Six (Phases D2–E, see `MODULE_6_REPORT.md`): inbound email matching (including reply-triggered sequence stopping), delivery-status webhooks, and a bulk-send UI.
