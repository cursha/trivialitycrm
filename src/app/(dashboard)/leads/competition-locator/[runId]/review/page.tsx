...
export default async function CompetitionLocatorReviewPage({ params }: { params: Promise<{ runId: string }> }) {
  const user = await requireUser();
  requirePermission(user, "review_research_results");
  const { runId } = await params;

  const results = await prisma.searchResult.findMany({
    where: { search: { runCorrelationId: runId } },
    include: {
      search: { include: { leadType: true, competitor: true } },
  ...
  const competitorName = results[0].search.competitor?.name ?? "Unknown competitor";
  const competitorId = results[0].search.competitorId;
  const leadTypeId = results[0].search.leadTypeId;
    ...
    return {
      ...
      matchedCompany: matchedCompany
        ? {
            ...
            competitorId: matchedCompany.competitorId,
            competitorName: matchedCompany.competitor?.name ?? null,
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <PageHeader title={`Review results: ${competitorName}`} description={`${results.length} result(s) found across this run.`} />
      <ResultsGroups
        runId={runId}
