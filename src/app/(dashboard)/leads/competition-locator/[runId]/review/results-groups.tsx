...
import { saveCompetitionLocatorResults, type CompetitionLocatorSaveResult } from "./actions";
import { FieldCompare, type ExistingRecord } from "./field-compare";
...
export function ResultsGroups({
  ...
  const [conflictResolutionByRow, setConflictResolutionByRow] = useState<Record<string, "assignAnyway" | "keepExisting" | "skip">>({});
  ...
  const [summary, setSummary] = useState<Exclude<CompetitionLocatorSaveResult, { error: string }> | null>(null);
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await saveCompetitionLocatorResults({
        runId,
        assignedToId,
        pipelineStageId,
        rows: Array.from(selected).map((id) => {
          ...
          return {
            ...
            competitorConflictResolution: row.bucket === "competitorConflict" ? (conflictResolutionByRow[id] ?? "keepExisting") : undefined,
          };
        }),
      });
      if ("error" in result) {
            ...
            <div className="space-y-3">
              {bucketRows.map((row) => (
                <ResultRowCard
                  ...
                  conflictResolution={conflictResolutionByRow[row.id] ?? "keepExisting"}
                  onConflictResolution={(value) => setConflictResolutionByRow((prev) => ({ ...prev, [row.id]: value }))}
                />
              ))}
            </div>
          </Card>
...
function ResultRowCard({
  ...
  conflictResolution: "assignAnyway" | "keepExisting" | "skip";
  onConflictResolution: (value: "assignAnyway" | "keepExisting" | "skip") => void;
}) {
    ...
    <div className="rounded-lg border border-border p-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {row.bucket === "competitorConflict" && row.matchedCompany && (
            <div className="mt-2 rounded-lg border border-danger/30 bg-danger/5 p-2">
              <p className="text-xs font-semibold text-text">
                {row.matchedCompany.name} already has a different competitor on file: {row.matchedCompany.competitorName ?? "(unknown)"}
              </p>
              <div className="mt-1 flex items-center gap-2">
                <Label className="text-xs uppercase text-text-muted">Action</Label>
                <Select
                  value={conflictResolution}
                  onChange={(e) => onConflictResolution(e.target.value as typeof conflictResolution)}
                  className="w-auto py-1 text-xs"
                >
                  <option value="keepExisting">Keep existing competitor</option>
                  <option value="assignAnyway">Assign this competitor anyway</option>
